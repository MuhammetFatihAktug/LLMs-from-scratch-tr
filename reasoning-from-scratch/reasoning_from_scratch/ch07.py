...
def download_from_github(rel_path, out=None):
    github_raw_base = (  # Temel URL
        "https://raw.githubusercontent.com/rasbt/"
        "reasoning-from-scratch/refs/heads/main/"
    )

    rel_path = Path(rel_path)
    # Varsayılan çıktı dosya adı olarak URL dosya adını kullan
    out = Path(out) if out is not None else Path(rel_path.name)

    # Dosya yerelde zaten varsa indirmeyi atla
    if out.exists():
...
def moving_average(values, window_fraction=0.25):
    # Eğitim sırasında daha uzun vadeli eğilimleri ortaya çıkarmak için gürültülü bir eğitim sinyalini düzleştir
...
def plot_grpo_metrics(csv_path, columns, save_as=None):
    ...
    # CSV günlük dosyasını aç ve oku
    with Path(csv_path).open(newline="") as f:
        ...
        for row in reader:
            ...
            # Tüm ölçütler için ortak x ekseni olarak eğitim adımını kullan
            step = int(row["step"])
    ...
    # Kayıp, ödüller, yanıt uzunluğu vb. yan yana gösterilebilsin diye sabit bir ızgara oluştur
    ...
    for i, name in enumerate(columns):
        ...
        # Bulunmayan ölçütleri atla
        if not values:
            fig.delaxes(axes[i])
            continue

        # Her adım için verimiz olmadığından değerlendirme doğruluğunu çubuk grafik olarak göster
        if name == "eval_acc":
...
def compute_advantage_stats(rewards_list):
    # GRPO'da zaten hesapladığımız şey bu:
    rewards = torch.tensor(rewards_list)
    advantages = (rewards - rewards.mean()) / (rewards.std() + 1e-4)

    # Eklediğimiz yeni istatistikler bunlar:
...
def sequence_logprob_and_entropy(model, token_ids, prompt_len):
    ...
    # Üretilen yanıt token'larının log-olasılığı (yanıt adımları üzerinden toplam)
    ...
    if all_answer_logprobs.numel() == 0:  # Model hemen EOS token'ı döndürürse diye koruma
        entropy_all_steps = logp_all_steps.new_tensor(0.0)
    else:
        all_answer_probs = torch.exp(all_answer_logprobs)  # logprob'u olasılığa dönüştür
        plogp = all_answer_probs * all_answer_logprobs     # öğe bazında p * log p
        step_entropy = -torch.sum(plogp, dim=-1)           # sözcük dağarcığı üzerinden topla -> adım başına entropi
        entropy_all_steps = torch.mean(step_entropy)       # yanıt adımları üzerinden ortalama al
