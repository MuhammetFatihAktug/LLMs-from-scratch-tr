...
def build_examples(data, tokenizer):
    ...
    for entry in data:
        try:
            ...
            # Adım 3: İstemi ve yanıtı birleştir
            token_ids = (
...
def train_distillation(
    ...
    # Adım 1: optimize ediciyi başlat (model zaten yüklü)
    ...
    # Adım 2: eğitim dönemleri üzerinde döngü kur
    for epoch in range(1, epochs + 1):
        # Adım 3: dönemin başında eğitim örneklerini karıştır
        epoch_examples = list(train_examples)
        rng.shuffle(epoch_examples)

        # Adım 4: dönemdeki eğitim örnekleri üzerinde döngü kur
        for example in epoch_examples:
            global_step += 1

            # Aşama 5: kayıp gradyanını sıfırla
            # (bunu her adımın başında yapmak iyi bir uygulamadır)
            optimizer.zero_grad()

            # Adım 6: mevcut örnek için çapraz entropi kaybını hesapla
            loss = compute_example_loss(model, example, device)

            # Adım 7: gradyanları geri yay
            loss.backward()

            # Eğitim kararlılığını artırmak için isteğe bağlı olarak büyük gradyanları kırp
            if grad_clip_norm is not None:
                torch.nn.utils.clip_grad_norm_(
                    model.parameters(), grad_clip_norm
                )

            # Adım 8: model ağırlıklarını güncelle
            optimizer.step()

            # Adım 9: mevcut modeli doğrulama kümesinde düzenli aralıklarla değerlendir
        ...
        # Adım 10: bu dönem için bir kontrol noktası kaydet
