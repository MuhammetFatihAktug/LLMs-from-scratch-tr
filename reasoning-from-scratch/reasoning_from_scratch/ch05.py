...
def heuristic_score(
    answer,
    prompt=None,  # Yok sayılan yer tutucu
    brevity_bonus=500.0,
    boxed_bonus=2.0,
    extract_bonus=1.0,
    fulltext_bonus=0.0,
    ...
    # Nihai boxed değeri içeren yanıtları ödüllendir
    cand = extract_final_candidate(answer, fallback=None)
    if cand:
        score += boxed_bonus

    # Yanıtta boxed değer yoksa daha zayıf ödül ver
    else:
    ...
    # Metin uzunluğuyla azalan bir kısalık ödülü ekle
...
def calc_next_token_probas(model, tokenizer, prompt, device):
    ...
    # Metin üretme fonksiyonlarına benzer şekilde logit'leri ve olasılıkları al
    logits = model(token_ids.unsqueeze(0)).squeeze(0)
    all_probas = torch.softmax(logits, dim=-1)

    # Puanladığımız konumlar (burada: tümü)
    t_idx = torch.arange(0, token_ids.shape[0] - 1, device=device)

    # Metin elimizde olduğu için gerçek sonraki token'ları biliyoruz
    next_ids = token_ids[1:]

    # Her sonraki token için olasılıkları al
    ...
    # Dizinin olabilirliği, olasılık puanlarının çarpımıdır
    print(
        "Joint probability:",
...
def calc_next_token_logprobas(model, tokenizer, prompt, device, show=True):
    ...
    logits = model(token_ids.unsqueeze(0)).squeeze(0)
    # Artık log_softmax kullanıyoruz
    ...
    # Çarpımı bir toplamla değiştiriyoruz
...
def self_refinement_loop(
    ...
    # Bir veya daha fazla yineleme çalıştır
    for it in range(iterations):
        ...
        if score_fn:
            revised_score = score_fn(
                answer=revised_full, prompt=prompt  # Burada hâlâ özgün istemi kullan
            )
        else:
            revised_score = 0.0
        ...
        # Gözden geçirilmiş yanıt daha kötü değilse kabul et
