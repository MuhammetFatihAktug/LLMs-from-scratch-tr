...
class Qwen3Model(nn.Module):
    ...
    def forward(self, in_idx, cache=None, attn_mask=None):
        ...
        # Varsa önbellek içeriğinden (katman 0'ın K uzunluğu) pos_start değerini türet
        ...
        # [Q=num_tokens, K=pos_end] için nedensel maske kur
        base = torch.triu(
        ...
        if has_pad:
            # Doldurulmuş anahtarları maskele ki softmax paydasında görünmesinler
        ...
        # Doldurulmuş sorgu satırlarını sıfırla; böylece Q/K/V sıfır olur ve önbelleği etkilemez
    ...
    # Yığınsız normal generate_text_basic_cache fonksiyonuyla uyumluluk için tutuluyor
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, mask, cos, sin, cache=None, pos_ids=None):
        ...
        shortcut = x
        x = self.norm1(x)
        x, next_cache = self.att(x, mask, cos, sin, cache=cache, pos_ids=pos_ids)  # Şekil [batch_size, num_tokens, emb_size]
...
class GroupedQueryAttention(nn.Module):
    ...
    def forward(self, x, mask, cos, sin, cache=None, pos_ids=None):
        ...
        # RoPE uygula (token başına konum kimlikleri)
        ...
        # Puan yolu yukarı dönüştürüldüyse bağlam matmul'ü için değer dtype'ıyla eşleş
...
def generate_text_basic_batched_cache(
    ...
    # Önbelleği ve model konumunu başlat
    cache = KVCache(n_layers=model.cfg["n_layers"])

    # Ön doldurma
    out = model(token_ids, cache=cache, attn_mask=attn_mask)[:, -1]

    # Hangi dizilerin zaten EOS ürettiğini izle
    if eos_token_id is not None:
        # Bir istem zaten EOS ile bitiyorsa onu tamamlanmış say
    ...
    for _ in range(max_new_tokens):
        # Tüm diziler zaten tamamlandıysa dur
        ...
        if eos_token_id is not None:
            # Şekli korumak için tamamlanmış satırları EOS üretmeye devam ettir
        ...
        # Maskeyi yeni üretilen token'ı kapsayacak şekilde genişlet
        ...
        # KV önbelleğiyle bir token ilerle
        out = model(next_token, cache=cache, attn_mask=cur_attn)[:, -1]
        generated_tokens.append(next_token)

        # Bu adımın token'ını ekledikten sonra tamamlanma maskesini güncelle
        if eos_token_id is not None:
...
def shrink_kv_cache_inplace(cache, keep_mask, n_layers):
    ...
    for i in range(n_layers):
        ...
        if kv is None:
            continue
        K, V = kv
        K = K[keep_mask]  # yığın boyutu boyunca küçült
        V = V[keep_mask]
        cache.update(i, (K, V))
...
def generate_text_basic_batched_cache_stop(
    ...
    # Dikkat maskesini kur
    if attn_mask is None and pad_id is not None:
        attn_mask = (token_ids != pad_id)
    if attn_mask is not None:
        attn_mask = attn_mask.to(torch.bool).to(device)

    # Önbelleği başlat ve tüm yığın üzerinde bir kez ön doldur
    cache = KVCache(n_layers=model.cfg["n_layers"])
    out = model(token_ids, cache=cache, attn_mask=attn_mask)[:, -1]  # (B, V)

    finished_full = torch.zeros(B, dtype=torch.bool, device=device)
    active_idx = torch.arange(B, device=device)  # etkin satırlar -> özgün satırlar
    cur_attn_active = attn_mask                  # etkin önbelleği yansıtır
    generated_full_steps = []                    # (B,1) adım tensörlerinin listesi

    for _ in range(max_new_tokens):
        # Etkin alt yığın için sonraki token'lar
        next_token_active = torch.argmax(out, dim=-1, keepdim=True)  # (B_active, 1)

        # Tam boyutlu (B,1) adım tensörüne dağıt (tamamlanmış satırlar için EOS)
        ...
        # Tamamlanma kayıtlarını tam yığın koordinatlarında güncelle
        if eos_token_id is not None:
        ...
        # Hesaplama yığınında yalnızca hayatta kalanları tut
        ...
        # Dikkat maskesini küçült ve üretilen token için bir "1" ekle
        ...
        # KV önbelleğini yığın boyutu boyunca hayatta kalanlara küçült
        shrink_kv_cache_inplace(cache, keep_mask_active, model.cfg["n_layers"])

        # Yalnızca hayatta kalanlar için bir token ilerle
        out = model(next_token_survivors, cache=cache, attn_mask=cur_attn_active)[:, -1]

    # Adım tensörlerini birleştir; yalnızca üretilen kısmı döndür
...
def generate_text_basic_batched_stream_cache_stop(
    ...
    for _ in range(max_new_tokens):
        ...
        # Döndürülecek tam boyutlu adımı kur
        fill_val = int(eos_token_id) if eos_token_id is not None else 0
        step_full = torch.full((B, 1), fill_value=fill_val,
                               dtype=token_ids.dtype, device=device)
        step_full.index_copy_(0, active_idx, next_token_active)
        ...
        # Küçültmeden önce döndür ki çağıranlar adım başına tam olarak bir (B,1) görsün
        yield step_full
