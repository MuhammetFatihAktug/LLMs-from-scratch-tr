...
def sequence_logprob_and_entropy(model, token_ids, prompt_len):
    ...
    # Her yanıt adımında tüm sözcük dağarcığı dağılımı üzerinden entropi
    all_answer_logprobs = logprobs[:-1][prompt_len - 1:]
    if all_answer_logprobs.numel() == 0:  # Model hemen EOS token'ı üretirse diye koruma
...
def compute_grpo_loss(
    ...
    for _ in range(num_rollouts):
        ...
        # Bu güncelleme için rollout politikasını "eski politika" olarak ele al (detach)
        with torch.no_grad():
    ...
    # Dizi log-olasılıklarını geçerli politika altında yeniden hesapla (gradyanlı)
    new_logps = []
...
def train_rlvr_grpo(
    ...
    try:
        for step in range(steps):
            ...
            # Önceki adımdaki modelin kopyasını al
            ...
            # Ardından birden çok kez yinele
