...
def apply_keep_sampling_mask(logits, keep_masks, prompt_len):
    ...
    for i, mask in enumerate(keep_masks):
        ...
        # (11) Örnekleme maskesini koru: eğitim sırasında örneklenen eylem alt uzayını zorunlu kıl
...
def sequence_logprob_and_entropy(model, token_ids, prompt_len, keep_masks=None, return_entropy=False):
    ...
    if keep_masks:
        # (11) Log-olasılık değerlendirmesi için örnekleme maskesini koru
...
def compute_grpo_loss_plus_kl(
    ...
    # (10) Politika dışı dizi maskeleme: büyük KL'li negatif avantajlı örnekleri maskele
...
if __name__ == "__main__":
    ...
    kl_coeff = args.kl_coeff  # (8) Matematik alanı için KL terimi varsayılan olarak sıfırdır
    if kl_coeff:
        # (8) KL terimine dair: ref_model yalnızca KL etkinken oluşturulur
