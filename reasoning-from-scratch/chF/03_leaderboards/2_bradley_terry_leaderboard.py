...
def bradley_terry_torch(vote_pairs, device):
    ...
    for epoch in range(500):
        s = scores()
        delta = s[winners] - s[losers]       # puan farkı
        loss = -torch.nn.functional.logsigmoid(delta).mean()   # negatif log-olabilirlik
        optimizer.zero_grad(set_to_none=True)
        loss.backward()
        optimizer.step()

    # Gizil puanları Elo benzeri ölçeğe dönüştür
    with torch.no_grad():
        s = scores()
        scale = 400.0 / math.log(10.0)
        R = s * scale
        R -= R.mean()
        R += 1000.0  # 1000 çevresinde ortala
