...
class DummyTokenizer:
    ...
    def encode(self, prompt):
        # İstemin içeriği bu testler için önemsizdir.
        return [1, 2]
...
def test_top_p_filter_truncates_and_renormalizes():
    ...
    # Süzme gerekmediğinde çıktı girdiyle eşleşmeli
...
def test_top_p_filter_batched_rows_renormalize_independently():
    # Yığınlı durumlarda da çalıştığından emin ol
    probas = torch.tensor(
        [
...
def test_self_consistency_vote_majority(monkeypatch):
    ...
    # Yanıtlar arasında belirlenimci biçimde dolaşabilmek için çağrı dizinini enjekte eden sarmalayıcı
    call_counter = {"i": 0}
