...
def test_generate_text_stream_concat(monkeypatch):
    # Gerçek hesaplamadan kaçınmak için alttaki akış üreticisini taklit et
    def fake_stream(**kwargs):
        for t in (1, 2, 1):
            yield torch.tensor([t])  # fonksiyondaki squeeze(0) ile eşleşir
...
def test_get_last_boxed():
    ...
    cases = [
        (r"foo \boxed{42}", "42"),
        (r"\boxed{a} bla \boxed{b+c}", "b+c"),   # sonuncuyu seçer
        (r"noise \boxed   {  x^2 } end", "  x^2 "),  # boşluklara izin verir
        (r"\boxed{outer {inner} ok}", "outer {inner} ok"),  # iç içe geçme
        (r"nothing here", None),  # boxed eksik
        (r"\boxed  not_brace", None),  # açılış { yok
        (r"\boxed{unbalanced", None),  # dengesiz
    ]
...
def test_normalize():
    cases = [
        ...
        # LaTeX matematik kipi ve boşluk
        ("$2/3$", "2/3"),
        (r"\( 2/3 \)", "2/3"),
        ...
        # Çoktan seçmeli şık etiketleri
        ("c. 3", "3"),
        ("b: 2", "2"),

        # Kökler (burada matematiği sadeleştirmiyoruz, yalnızca metni normalleştiriyoruz)
        (r"\sqrt{2}", "sqrt(2)"),

        # Süslü parantez kaldırma
        (r"{x}", "x"),
        (r"{ (1, 2) }", "(1, 2)"),
    ]
...
def test_sympy_parser():
    ...
    failure_cases = [
        ...
        "??",  # geçersiz token'lar
        "2**",  # eksik işleç
        None,  # eksik girdilere karşı koruma
        long_expr,  # aşırı uzun girdilere karşı koruma
    ]
