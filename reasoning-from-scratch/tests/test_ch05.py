...
def test_self_refinement_loop_accepts_improving_revisions(monkeypatch):
    responses = iter(
        [
            "initial draft",                # ilk üretim
            "first critique",               # eleştiri 1
            "draft with more detail",       # iyileştirme 1 (kabul edildi)
            "second critique",              # eleştiri 2
            "bad",                          # iyileştirme 2 (reddedildi)
        ]
    )
    prompts_seen = []
