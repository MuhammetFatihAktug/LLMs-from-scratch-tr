...
# generate_text_basic testleri için sahte model.
class DummyModel:
    ...
    def __call__(self, token_ids, cache=None):
        ...
        # argmax'in fixed_token döndürmesi için fixed_token sütununu en yüksek değere ayarla
...
def test_get_device_tf32_setting_is_compatible_with_torch_compile():
    ...
    # PyTorch 2.9 ve 2.10, torch.compile içinde hâlâ eski allow_tf32 bayrağını okuyor.
    # get_device() içinde yeni API'yi kullanmak karışık API hatası tetikliyor.
    code = """
import torch
...
def test_generate_text_basic_stops_on_eos():
    ...
    token_ids = torch.tensor([[1, 3, 4]])  # şekil (batch, seq_len)

    # Üretimin hemen durması için eos_token_id'yi fixed_token olarak ayarla
