...
# Sohbet şablonunu daha sonra elle uygulayacağız
...
# Akıl yürütme modeli <|im_end|> üretebilir; temel model <|endoftext|> üretebilir.
EOS_TOKEN_IDS = (
...
# Rol-içerik sözlüklerinden oluşan liste hâlinde çok turlu bellek
...
def run_generate(user_text):
    # Kullanıcı istemini geçmişe ekle
    history.append({"role": "user", "content": user_text})

    # Geçmişin tamamını kodla
    ...
    # Baştan kırp (üretime yer açmak için)
    ...
    for tok in generate_text_basic_stream_cache(
        ...
        if token_id in EOS_TOKEN_IDS:  # Durdurma token'larında elle kes
            break
...
# Etkileşimli REPL (Oku, Değerlendir, Yazdır, Döngü)
try:
    while True:
        try:
            user_in = input(">> ").strip()
        ...
        if low in {r"\exit", r"\quit"}:
            break
        if low == r"\clear":
            # Geçmişi sıfırla ama sistem istemini koru
            ...
            continue
        if low == r"\history":
            # Asistan turlarını, şimdiye kadarki model yanıtı sayısı olarak say
