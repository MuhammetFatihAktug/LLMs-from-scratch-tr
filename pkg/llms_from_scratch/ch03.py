...
class CausalAttention(nn.Module):
    ...
    def __init__(self, d_in, d_out, context_length,
        ...
        self.W_key = nn.Linear(d_in, d_out, bias=qkv_bias)
        self.W_value = nn.Linear(d_in, d_out, bias=qkv_bias)
        self.dropout = nn.Dropout(dropout)  # Yeni
        self.register_buffer("mask", torch.triu(torch.ones(context_length, context_length), diagonal=1)) # Yeni

    def forward(self, x):
        b, num_tokens, d_in = x.shape  # Yeni yığın (batch) boyutu b
        ...
        attn_weights = self.dropout(attn_weights)  # Yeni
